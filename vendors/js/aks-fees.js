...
$(function(){
	...
	$(document).on('click','#btn-save-fees',function(){  
		...
		if (pp_fees == false || cc_fees == false) alert("This is not a number or empty field")
		else {
	...
	$(document).on('change', '.input-range', function() {
		...
		if(target == 'append-pp-fees-body') sortTable('#append-pp-fees-body', 'asc');
		else if(target == 'append-cc-fees-body') sortTable('#append-cc-fees-body', 'asc');
	});
...
function AjaxStores($_query = false){
	$id_to_update = null;
	if(!$_query) var $_query = { action : "aks-fees-merchant" , site : 'aks'}
...
function sortTable($table, $order) {
	...
	var $rows = $($table+' > tr');
		$rows.sort(function (a, b) {
			...
			if ($order=='asc') {
				if(!isNaN(keyA) && !isNaN(keyB)) return keyA - keyB; //If compare(a,b) is less than zero, the sort() method sorts a to a lower index than b. In other words, a will come first.
				if((keyA || keyB) == 'infinity') return 1;
			}
			if ($order =='desc') {
				if(!isNaN(keyA) && !isNaN(keyB)) return keyB - keyA; //If compare(a,b) is greater than zero, the sort() method sort b to a lower index than a, i.e., b will come first.
				if((keyA || keyB) == 'infinity') return 1;
			}
		});
